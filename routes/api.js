...
var creator = 'Specstor' // ubah jadi nama lu
const listkey = ["Free","Specstor","Ryan"]; // ubah apikey nya, tambah aja klo mau
...
router.get('/download/yt/mp3', async (req, res, next) => {
          var apikey = req.query.apikey
          var url = req.query.url
       	if(!apikey) return res.sendFile(__path + '/docs/403.html')
       if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
...
router.get("/download/yt/playmp3", async(req, res, next) => {
...
router.get("/download/yt/playmp4", async(req, res, next) => {
